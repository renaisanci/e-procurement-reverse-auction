...
shoppingCart.prototype.loadItems = function () {
    var itemss = localStorage != null ? localStorage[this.cartName + "_items"] : null;
    if (itemss != null && JSON != null) {
        try {
            ...
            for (var i = 0; i < items.length; i++) {
                ...
                if (item.sku != null && item.name != null && item.quantity != null && item.imagem != null) {
                    item = new cartItemCota(item.sku, item.name, item.price, item.quantity, item.imagem, item.flgOutraMarca);
...
shoppingCart.prototype.addItem = function (sku, name, price, quantity, imagem) {

    if (localStorage["fornRemPedCot"] != null && localStorage["fornRemPedCot"] != undefined) {

        var fornRemPedCot = JSON.parse(localStorage["fornRemPedCot"]);
    }
    if (quantity <= 0 && fornRemPedCot != undefined) {
        fornRemPedCot = fornRemPedCot.filter(function (returnableObjects) {
            return returnableObjects.prd !== sku;
        });
        localStorage["fornRemPedCot"] = JSON.stringify(fornRemPedCot);
    }
    ...
    if (quantity != 0) {
        ...
        for (var i = 0; i < this.items.length && !found; i++) {
            ...
            if (item.sku == sku) {
                ...
                item.quantity = this.toNumber(quantidade + quantity);
                if (item.quantity <= 0) {
        ...
        // new item, add now
        if (!found) {
            var itemAddNow = new cartItemCota(sku, name, price, quantity, imagem, true);
            this.items.push(itemAddNow);
        }
...
// items in the cart
//
function cartItemCota(sku, name, price, quantity, imagem, flgOutraMarca) {
    this.sku = sku;
    this.name = name;
    this.price = price * 1;
    ...
    this.imagem = imagem;
    this.qtdForn = 0;
    this.flgOutraMarca = flgOutraMarca;
}
