	...
	.directive("multiselect", ["$parse", "$document", "$compile", "$interpolate", "optionParser", function($parse, $document, $compile, $interpolate, optionParser) {
		return {
			...
			link    : function(originalScope, element, attrs, modelCtrl) {
			    ...
			    var textSelec = attrs.textselect;
				...
				function getHeaderText() {
					if(isEmpty(modelCtrl.$modelValue)) {
					    scope.header = attrs.msHeader || textSelec;
						return scope.header;
					}
					...
					if(isMultiple) {
						if(attrs.msSelected) {
							...
							scope.header = modelCtrl.$modelValue.length + " " + "Fabricante(s) Selecionados";
						}
...
angular.module("multiselect.tpl.html", []).run(["$templateCache", function($templateCache) {
  ...
  // " <button aria-expanded=\"false\"  tabindex=\"{{tabindex}}\" title=\"{{header}}\"type=\"button\"  ng-click=\"toggleSelect()\" class=\"multiselect dropdown-toggle btn btn-white btn-primary\" ng-disabled=\"disabled\" ng-class=\"{'error': !valid()}\" data-toggle=\"dropdown\"><span class=\"multiselect-selected-text\">None selected</span> &nbsp;<b class=\"fa fa-caret-down\"></b>\n" +

    "  <button tabindex=\"{{tabindex}}\" title=\"{{header}}\" type=\"button\" class=\" btn btn-white btn-grey dropdown-toggle\" ng-click=\"toggleSelect()\" ng-disabled=\"disabled\"  style=\"height: 40px;\" ng-class=\"{'error': !valid()}\">\n" +
			"    <span class=\"multiselect-selected-text\">{{header}}</span> &nbsp;<b class=\"fa fa-caret-down\"></b>\n" +
			"  </button>\n" +
